    ...
    if (ifChangeOrder) {
        for (it = ProblematicLights.rbegin(); it != ProblematicLights.rend(); ++it) {
            ...
            if (rand() % 100 > 60 && newSolution->Lights[index]->numberOfUsedStreets >
                                     1) { // żeby było trochę bardziej stochastycznie + wiadomo nie mutować ulic tylko z 1 used str
...
Solution *Solver::dropThosePeskyCars(Solution *solution,
                                     float thereshold) { // like cars that take up no more than 1.3 of time, can be set to 1.5 and then increased/ decreased, best to start from quite a high number like 2
